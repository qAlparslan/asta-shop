...
const STATUS_COPY = {
    ...
    kargolandi: {
        ...
        message: 'Siparişiniz DHL ile yola çıktı. Takip numaranız ve bağlantı aşağıdadır.',
        pillBg: T.brandMuted,
        pillColor: T.brand,
        border: '#f5d0d6',
    },
...
module.exports = function orderStatusUpdateTemplate({ order, newStatus, storeName, logoUrl, frontendUrl }) {
    ...
    const trackUrl =
        newStatus === 'kargolandi' && trackingNo
            ? buildPublicTrackingUrl(order.carrier || 'DHL', trackingNo)
            : null;
    ...
    <div style="${T.cardSoft} margin:22px 0;">
      <h3 style="${T.labelUpper}">DHL kargo takibi</h3>
      ...
      ${trackUrl ? renderCtaButton('Kargonuzu DHL üzerinde takip edin', trackUrl) : ''}
    </div>`
            : '';
